# Hyperparameter Summary - experiment_6

## Experiment Note

Sequential tuning after experiment_5. experiment_5에서 clip_vit_b32 + linear_svm이 가장 높은 CV Macro F1과 낮은 std를 보였으므로, Linear SVM의 C를 10.0에서 30.0으로 추가 증가시켜 더 약한 regularization이 계속 성능을 향상시키는지 확인한다. 다른 모델들은 baseline으로 유지한다.
...
- Change note: experiment_5의 C=10.0에서 C=30.0으로 추가 증가
- Important parameters: `{"estimator__C": 30.0, "estimator__class_weight": null, "estimator__max_iter": 5000, "cv": 3}`
